...
void vp8_build_inter4x4_predictors_mbuv(MACROBLOCKD *x)
{
    ...
    for (i = 16; i < 24; i += 2)
    {
        ...
        if (d0->bmi.mv.as_int == d1->bmi.mv.as_int)
            build_inter_predictors2b(x, d0, d0->predictor_base + d0->predictor_offset, 8);
        else
        {
...
static void build_inter4x4_predictors_mb_e(MACROBLOCKD *x)
{
    ...
    if (x->mode_info_context->mbmi.partitioning < 3)
    {
        ...
        build_inter_predictors4b(x, &x->block[ 0], x->block[ 0].predictor_base + x->block[ 0].predictor_offset, 16);
        build_inter_predictors4b(x, &x->block[ 2], x->block[ 2].predictor_base + x->block[ 2].predictor_offset, 16);
        build_inter_predictors4b(x, &x->block[ 8], x->block[ 8].predictor_base + x->block[ 8].predictor_offset, 16);
        build_inter_predictors4b(x, &x->block[10], x->block[10].predictor_base + x->block[10].predictor_offset, 16);
    }
    else
    {
        for (i = 0; i < 16; i += 2)
        {
            ...
            if (d0->bmi.mv.as_int == d1->bmi.mv.as_int)
                build_inter_predictors2b(x, d0, d0->predictor_base + d0->predictor_offset, 16);
            else
            {
                build_inter_predictors_b(d0, d0->predictor_base + d0->predictor_offset, 16, x->subpixel_predict);
                build_inter_predictors_b(d1, d1->predictor_base + d1->predictor_offset, 16, x->subpixel_predict);
            }
    ...
    for (i = 16; i < 24; i += 2)
    {
        ...
        {
            build_inter_predictors_b(d0, d0->predictor_base + d0->predictor_offset, 8, x->subpixel_predict);
            build_inter_predictors_b(d1, d1->predictor_base + d1->predictor_offset, 8, x->subpixel_predict);
        }
    }
}
